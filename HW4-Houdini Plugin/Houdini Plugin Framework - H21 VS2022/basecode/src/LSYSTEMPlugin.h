...
private:
    ...
    int    GENERATIONS(fpreal t) { return evalInt("generations", 0, t); }

    void   GRAMMAR(UT_String& str, fpreal t)
    {
        evalString(str, "grammar", 0, t);
    }

    UT_Vector3 START(fpreal t)
    {
        UT_Vector3 v;
        v.x() = evalFloat("start", 0, t);
        v.y() = evalFloat("start", 1, t);
        v.z() = evalFloat("start", 2, t);
        return v;
    }
