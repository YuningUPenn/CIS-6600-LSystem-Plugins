...
static PRM_Name genName("generations", "Generations");
...
// Default values
static PRM_Default angleDefault(30.0f);
static PRM_Default stepDefault(1.0f);
static PRM_Default genDefault(3);
static PRM_Default grammarDefault(0, "");

// Ranges for UI sliders
static PRM_Range angleRange(PRM_RANGE_RESTRICTED, 0, PRM_RANGE_RESTRICTED, 180);
static PRM_Range stepRange(PRM_RANGE_RESTRICTED, 0.0001, PRM_RANGE_UI, 10);

static PRM_Name startName("start", "Start Position");
static PRM_Default startDefault[3] = {
	PRM_Default(0.0f),
	PRM_Default(0.0f),
	PRM_Default(0.0f)
};
...
// Similarly add all the other parameters in the template format here
	...
	// Angle (float)
	PRM_Template(PRM_FLT, 1, &angleName, &angleDefault, 0, &angleRange),

	// Step length (float)
	PRM_Template(PRM_FLT, 1, &stepName, &stepDefault, 0, &stepRange),

	// Generations (int)
	PRM_Template(PRM_INT, 1, &genName, &genDefault),

	// Grammar file path
	PRM_Template(PRM_FILE, 1, &grammarName, &grammarDefault),

	// Start Position
	PRM_Template(PRM_XYZ, 3, &startName, startDefault),
...
SOP_Lsystem::cookMySop(OP_Context &context)
{
	...
	// Get input geometry
	const GU_Detail* input_gdp = inputGeo(0, context);
	bool hasInput = (nInputs() > 0 && input_gdp && input_gdp->getNumPoints() > 0);

	// Read parameters from UI
	float angle = ANGLE(now);
	float step = STEP(now);
	int generations = GENERATIONS(now);

	UT_String grammar;
	GRAMMAR(grammar, now);

	UT_Vector3 startPos = START(now);
	...
	myplant.process(generations, branches);
	...
	if (boss->opStart("Building LSYSTEM"))
	{
		...
		for (const auto &b : branches) {
			...
			gdp->setPos3(p0, UT_Vector3(b.first[0], b.first[1], b.first[2]));
			
			GA_Offset p1 = poly->getPointOffset(1);
			gdp->setPos3(p1, UT_Vector3(b.second[0], b.second[1], b.second[2]));
		}
		*/
		...
		for (const auto& b : branches)
		{
			if (!hasInput)
			{
				...
				GA_Offset p0 = poly->getPointOffset(0);
				gdp->setPos3(p0, startPos + UT_Vector3(b.first[0], b.first[1], b.first[2]));

				GA_Offset p1 = poly->getPointOffset(1);
				gdp->setPos3(p1, startPos + UT_Vector3(b.second[0], b.second[1], b.second[2]));
			}
			else
			{
				GU_Detail copy;
				...
				vec3 dir = b.second - b.first;
				dir.Normalize();
				vec3 mid = (b.first + b.second) * 0.5f;
				
				UT_Vector3 yaxis(0,1,0);
				UT_Vector3 target(dir[0], dir[1], dir[2]);
				UT_Matrix3 rot;
				rot.dihedral(yaxis, target);
				
				UT_Matrix4 xform(rot);
				xform.translate(startPos + UT_Vector3(mid[0], mid[1], mid[2]));
