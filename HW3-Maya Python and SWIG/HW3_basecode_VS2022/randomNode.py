...
class randomNode(OpenMayaMPx.MPxNode):
    ...
    def compute(self,plug,data):
        ...
        minVals = data.inputValue(randomNode.inMinBounds).asFloat3()
        maxVals = data.inputValue(randomNode.inMaxBounds).asFloat3()
        minX, minY, minZ = minVals
        maxX, maxY, maxZ = maxVals
        
        pointsData = data.outputValue(plug)
        pointsAAD = OpenMaya.MFnArrayAttrsData()
        pointsObject = pointsAAD.create()

        posArray = pointsAAD.vectorArray("position")
        ...
        for i in range(numPoints):
            ...
            posArray.append(OpenMaya.MVector(x, y, z))
            idArray.append(i)
...
def nodeInitializer():
    ...
    randomNode.inMinBounds = nAttr.createPoint("minBounds", "minb")
    MAKE_INPUT(nAttr)

    randomNode.inMaxBounds = nAttr.createPoint("maxBounds", "maxb")
    MAKE_INPUT(nAttr)
