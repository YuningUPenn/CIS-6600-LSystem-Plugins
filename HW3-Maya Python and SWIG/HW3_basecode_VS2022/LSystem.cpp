...
void LSystem::processPy(unsigned int n,
    ...
    for (auto& b : preBranches) {
        std::vector<float> branch;
        branch.push_back(b.first[0]);
        branch.push_back(b.first[1]);
        branch.push_back(b.first[2]);
        branch.push_back(b.second[0]);
        branch.push_back(b.second[1]);
        branch.push_back(b.second[2]);
        branches.push_back(branch);
    }

    for (auto& f : preFlowers) {
        if (f.second == "*") {
            std::vector<float> flower;
            flower.push_back(f.first[0]);
            flower.push_back(f.first[1]);
            flower.push_back(f.first[2]);
    ...
    std::vector<Geometry>& models)
{
    ...
    //turtle.applyLeftRot(-90); Incorrect rotation
